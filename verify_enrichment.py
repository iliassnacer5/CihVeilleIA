...
async def verify_enrichment():
    ...
    count = await orchestrator.run_single_source(source_id, config, limit=1) # Just one to be fast

    logger.info(f"Scraped {count} items for {source_id}")

    # Check the last document in MongoDB
    last_doc = await doc_store.collection.find_one({"source_id": source_id}, sort=[("created_at", -1)])
    if last_doc:
